...
class SitemapFileCustom extends SitemapFile{
   ...
   * @return void
   */
  public function addEntry($entry, $priority="0.8")
  {
    ...
    if(!in_array($entryMdf, $arDisallowRules)){   
      ...
      else
      {
        ...
        switch(Converter::getXmlConverter()->encode($entry['XML_LOC'])) {
          case "https://dev.plitkanadom.ru/":
          case "https://www.plitkanadom.ru/":
            $priority = "1";
            break;
        }

        /*if(strpos(Converter::getXmlConverter()->encode($entry['XML_LOC']), "/articles/") !== false) {
          $priority = "0.7";
        }*/
        $this->putContents(
          sprintf(
            ...
            date('c', strtotime(date("d").".".date("m").".".date("Y")) - \CTimeZone::getOffset()),
            $priority
          ), self::APPEND
        );
      }
   ...
   * @return void
   */
  public function addIBlockEntry($url, $modifiedDate, $priority="0.8")
  {
    $this->addEntry(array(
  ...
  */
  ...
  /**
   * Обертка над фукнцией str_replace
