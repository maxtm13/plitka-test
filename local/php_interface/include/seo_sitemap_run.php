<?
//ƒл€ запуска необходимой карты укажите в адресе ее ID, пример:
        ...
        elseif(is_array($arCurrentIBlock)) {
          ...
          if(isset($NS['CURRENT_SECTION'])) {
            ...
            if($arElement) {
              ...
              $priority = "0.8";
              /*if(strpos($arElement['DETAIL_PAGE_URL'], "/catalog/") !== false) {
                $priority = "0.8";
              }*/
            ...
            if($arSection) {
              ...
              if($bActive) {
                ...
                $priority = "0.8";
                /*if(strpos($arSection['SECTION_PAGE_URL'], "/catalog/") !== false) {
                  $priority = "0.9";
                }*/
                $sitemapFile->addIBlockEntry($url, $sectionLastmod, $priority);
              }
            ...
            elseif(!$bCheckFinished) {
              ...
              if($arSitemap['SETTINGS']['IBLOCK_LIST'][ $iblockId ] == 'Y' && strlen($arCurrentIBlock['LIST_PAGE_URL']) > 0) {
                ...
                $sitemapFile->addIBlockEntry($url, $NS['IBLOCK_LASTMOD'], "0.8");
              }
        ...
        elseif(is_array($arCurrentForum)) {
          ...
          if($bActive) {
            ...
            if(($arTopic = $dbTopicResult->fetch()) && $arTopic) {
              ...
              $sitemapFile->addIBlockEntry($url, MakeTimeStamp($arTopic['LAST_POST_DATE']), "0.8");
            }
          }
          else {
            $url = CForumNew::PreparePath2Message(
            ...
            $sitemapFile->addIBlockEntry($url, MakeTimeStamp($arCurrentForum['LAST_POST_DATE']), "0.8");
          }
          if(empty($arTopic)) {
            $bCheckFinished = false;
