...
<div class="comments_block">
	<? if($arResult['SHOW_COUNT'] == 'Y'):?>
		<p class="comments_block_title"><?=GetMessage('COMMENTS_COUNT')?> (<?=$arResult['COMMENTS_COUNT']?>)</p>
	<? endif; ?>
	<div class="comments_list">
	<? foreach($arResult['COMMENTS'] as $COMMENT): ?>
	<? if((($COMMENT['ACTIVATED'] == 0 && $arResult['GROUPS']) || $arResult['ISADMIN']) || $COMMENT['ACTIVATED'] == 1): ?>
		<div class="comment_item <? if($COMMENT['ACTIVATED'] == 0 && ($arResult['GROUPS'] || $arResult['ISADMIN'])): echo "not_approved"; endif; ?>" <? if(isset($COMMENT['LEFT_MARGIN'])) { ?>style="margin-left:<?=$COMMENT['LEFT_MARGIN']?>px;"<? } ?> id="comment_<?=$COMMENT['ID']?>">
			<div <? if($arResult['SHOW_USERPIC'] == "Y"): ?> class="user_icon" <? endif; ?>>
				...
				<? else: ?><img width="48" height="48" src="https://www.plitkanadom.ru/upload/iblock/files/nophoto.gif" alt="<?=$COMMENT['USER']['LOGIN']?>" /><? endif; ?>
                </p>
				<? endif; ?>
			</div>
			...
			<? if($COMMENT['USER']['LOGIN']): ?>
				<div class="comment_item_top" <? if($arResult['SHOW_USERPIC'] != "Y"): echo 'style="margin-left:0px;"'; endif; ?>><p><?=$COMMENT['USER']['LOGIN']?></p><? if($arResult['SHOW_DATE'] == 'Y'): ?>, <?=$COMMENT['DATE_CREATE'] ?><? endif; ?></div>
...
<form <? if($arResult['CURRENT_USER'] != 0): ?>onsubmit="document.getElementById('submitButton').disabled=true;document.getElementById('submitButton').style.cursor='wait';" <? endif; ?> method="post" action="" id="new_comment_form" style="display: none;" >
	<fieldset id="addform" style="border: none;"> 
		...
		<? endif; ?>
		<p><span class="star-required">*&nbsp;</span><?= GetMessage("NAME"); ?>:</p><input class="blured" id="AUTHOR_NAME" value="<?= $arResult['saveName'] ?>" type="text" name="AUTHOR_NAME" />
		<p><?= GetMessage("EMAIL"); ?>:</p><input id="EMAIL" value="<?= $arResult['EMAIL'] ?>" type="text" name="EMAIL" />
		<? endif; ?>
		<p><span class="star-required">*&nbsp;</span><?= GetMessage("YOUR_COMMENT"); ?>:</p>
		...
		<p><span class="star-required">*&nbsp;</span><?= GetMessage("CAPTCHA"); ?>:</p>
