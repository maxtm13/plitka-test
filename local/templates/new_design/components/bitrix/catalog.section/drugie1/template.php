...
?>
<link rel="stylesheet" type="text/css" href="/bitrix/templates/eshop_adapt_blue/js/slick/slick.css"/>
<link rel="stylesheet" type="text/css" href="/bitrix/templates/eshop_adapt_blue/js/slick/slick-theme.css"/>
<script type="text/javascript" src="/bitrix/templates/eshop_adapt_blue/js/slick/slick.min.js"></script>
<span class="other_product">Другие товары серии <?/*=$arResult['NAME'];*/?></span>
<?
...
?><div class="multiple-items bx_catalog_list_home col<? echo $arParams['LINE_ELEMENT_COUNT']; ?> <? echo $templateData['TEMPLATE_CLASS']; ?>"><?
...
<?if($arItem['PROPERTIES']['DISCOUNT']['VALUE'] == 3):?>
              <span class="bx_stick_disc right bottom"><?echo '-'; echo $arItem ['PRICES']['BASE']['DISCOUNT_DIFF_PERCENT']; echo '%'; ?></span>
              <?endif;?>
...
<?if (!empty($arItem['MIN_PRICE'])):?>
	...
	<div class="item_old_price"><? echo $arItem['MIN_PRICE']['VALUE']; ?></div>
	<?endif;?>
<?endif;?>	
	...
	if (!empty($arItem['MIN_PRICE']))
	{
		...
		if ('Y' == $arParams['SHOW_OLD_PRICE'] && $arItem['MIN_PRICE']['DISCOUNT_VALUE'] < $arItem['MIN_PRICE']['VALUE'])
		{
			?> <span class="item_old_price"><? echo $arItem['MIN_PRICE']['PRINT_VALUE']; ?><?echo $arItem['CATALOG_MEASURE_NAME']; ?></span>
			<?
		}
	}
	?><div><?echo $arItem['CATALOG_MEASURE_NAME']; ?></div>
	</div></div><?
...
<script type="text/javascript">
	$('.multiple-items').slick({
	  ...
	  slidesToShow: 4,
	  slidesToScroll: 4,
	  prevArrow: '<button id="prev" type="button" class="btn btn-juliet"><img src="/upload/prim/5.png"></button>',
	  nextArrow: '<button id="next" type="button" class="btn btn-juliet"><img src="/upload/prim/6.png"></button>',
	  responsive: [
	    {
	      breakpoint: 1025,
	      settings: {
