...
<div class="bx_content_section">
		...
		<? foreach($arResult["SECTIONS"] as $sect): ?>
				...
				<li class="lvl1">
					...
					<a href="<?=$sect["URL"];?>/<?=$arParams["BRAND"]["CODE"];?>" class="bx_catalog_tile_img" title="<?=$sect["NAME"];?>"><img src="<?=$sect["PICTURE"];?>" alt="<?=$sect["NAME"];?>" title="<?=$sect["NAME"];?>" class="catalog_item_img"></a>
					<a class="bx_catalog_tile_title" href="<?=$sect["URL"];?>/<?=$arParams["BRAND"]["CODE"];?>"><?=$arParams["BRAND"]["NAME"];?> <?=$sect["NAME"];?></a>
				</li>
				<? endif; ?>
		<?	endforeach; ?>
			</ul>
...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	?><div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>"><div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
		<div class="prop-icon">
			<? if($arItem['PROPERTIES']['DISCOUNT']['VALUE'] == 1):?>
				<div class="stickers">
					<span class="sticker_free_shipping_stock" title="<?=getMessage('DICOUNT_TITLE');?>"><?=getMessage('DICOUNT_TITLE1_STICKER');?></span>
				</div>
			<? endif; ?>
			<? if($arItem['PROPERTIES']['DISCOUNT']['VALUE'] == 2):?>
				<div class="stickers">
...
?>
<?
if (empty(trim($sotbitSeoMetaBottomDesc)) && empty($_GET)){
