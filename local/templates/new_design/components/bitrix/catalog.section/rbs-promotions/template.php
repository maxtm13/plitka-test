...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	<div class="bx_catalog_item_price">
        ...
        if (($arItem['PROPERTIES']['OLD_PRICE']['VALUE']) > ($arItem['MIN_PRICE']['VALUE']) && (empty($arItem['PROPERTIES']['DISCOUNT_PERCENT']['VALUE']))) { ?>
            <div class="tim_old_pice"><? echo $arItem['PROPERTIES']['OLD_PRICE']['VALUE'];?>
                <? echo GetMessage('OLD_PRICE_MESS_CUR');?></div>
        <? };?>
		...
		<div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price">
			<?if (!empty($arItem['PROPERTIES']['AVAILABILITY']['VALUE_ENUM_ID']) && in_array($arItem['PROPERTIES']['AVAILABILITY']['VALUE_ENUM_ID'], array(4914, 5044, 5649))) {
			...
			} else if (!empty($arItem['ITEM_PRICES'][0]["PRICE"])) {
				if ('N' == $arParams['PRODUCT_DISPLAY_MODE'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
					echo GetMessage(
						'CT_BCS_TPL_MESS_PRICE_SIMPLE_MODE',
						array(
							'#PRICE#' => $arItem['ITEM_PRICES'][0]["PRINT_PRICE"],
							'#MEASURE#' => GetMessage(
				...
				/*
				if ('Y' == $arParams['SHOW_OLD_PRICE'] && $arItem['ITEM_PRICES'][0]["PRINT_DISCOUNT"] < $arItem['ITEM_PRICES'][0]["PRICE"]) {
					?> <span><? echo $arItem['ITEM_PRICES'][0]["PRINT_PRICE"]; ?></span><?
				}
				*/
			} ?>
		</div>
