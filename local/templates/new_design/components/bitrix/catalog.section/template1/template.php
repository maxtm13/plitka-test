...
if (!empty($arResult['ITEMS']) && $arResult['DEPTH_LEVEL'] == 3){?>
	<?
		if(CModule::IncludeModule("catalog") && CModule::IncludeModule("iblock")){
			  $IBLOCK_ID = 4;
			  $SECTION_ID = $_REQUEST["SECTION_ID"];
			  $arFilter = Array('IBLOCK_ID'=>$IBLOCK_ID, 'ID'=>$SECTION_ID, 'GLOBAL_ACTIVE'=>'Y');
			  $arParams = array("UF_MORE_PHOTO");
			  $db_list = CIBlockSection::GetList(Array($by=>$order), $arFilter, $arParams);
			  while($ar_result = $db_list->GetNext())
			  {
				//print_r($ar_result);
				$description = $ar_result['DESCRIPTION'];
				$picture_id = $ar_result['PICTURE'];
			  }
		}
	?>
	
		<div class="image_wrap">
			<? echo CFile::ShowImage($picture_id, 250, 250, "border=0", "", true); ?>
		</div>
		<div class="price">Цена: <span>509</span> руб.</div>
		<? echo $description; ?>
	<div class="clear"></div>
	<div class="slash_devisor" style="margin-bottom:20px;margin-top:5px;"></div>
	<ul class="unit_wrap">
		<?foreach ($arResult['ITEMS'] as $key => $arItem)
		{
			if($arItem["PREVIEW_PICTURE"]["HEIGHT"] > 230){
				$arItem["PREVIEW_PICTURE"]["WIDTH"] = $arItem["PREVIEW_PICTURE"]["WIDTH"] * (230/$arItem["PREVIEW_PICTURE"]["HEIGHT"]);
				$arItem["PREVIEW_PICTURE"]["HEIGHT"] = 230;
			}
			if($arItem["PREVIEW_PICTURE"]["WIDTH"] > 255){
				$arItem["PREVIEW_PICTURE"]["HEIGHT"] = $arItem["PREVIEW_PICTURE"]["HEIGHT"] * (255/$arItem["PREVIEW_PICTURE"]["WIDTH"]);
				$arItem["PREVIEW_PICTURE"]["WIDTH"] = 255;
			}
			$count = 0;
		?>			
			<li style="text-align:center;">
				<!--h1><?=$arItem["NAME"]?></h1-->
				<div class="image_goods_wrap">
					<a class="group2" href="<? echo $arItem['PREVIEW_PICTURE']['SRC'];?>">
						<img src="<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>" alt = "Изображение плитки" width = "<?=$arItem["PREVIEW_PICTURE"]["WIDTH"]?>" height = "<?=$arItem["PREVIEW_PICTURE"]["HEIGHT"]?>" />
					</a>	
				</div>
				<?
					$PRODUCT_ID = $arItem["ID"];
					$asd = GetCatalogProductPriceList($PRODUCT_ID, "SORT", "ASC");
					
				?>
				<? if(count($asd) > 0){ ?>
					<input type = "hidden" class = "price_<?php echo $count ?>" value = "<? echo $asd[0]['PRICE'] ?>" />
				<? 
					$count ++;
				} ?>
			<div class="bottom_goods">
				<div class="left_part">
					<span style="color:#cd6902;">Цена: <? echo $asd[0]['PRICE'] ?></span><br/>
					Размер: <?=$arItem['PROPERTIES']['SIZE']['VALUE'];  ?><br/>
					Ед. измерения: <?=$arItem['PROPERTIES']['UNIT']['VALUE'];?><br/>
					<span style="position: relative;top: 3px;">Количество:</span> <input type="text" class="bx_col_input" id="<? echo $arItemIDs['QUANTITY']; ?>" name="<? echo $arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>" value="<? echo $arItem['CATALOG_MEASURE_RATIO']; ?>">
				</div>
				<div class="right_part">
					<img src="/bitrix/templates/eshop_adapt_blue/images/find-less.png" alt="find-less" class="fl" />
					<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="bx_bt_button bx_medium" href="javascript:void(0)" rel="nofollow">
						<img src="/bitrix/templates/eshop_adapt_blue/images/bt-orange-buy.png" alt="buy-button" />
					</a>
				</div>
			</div>
			</li>
...
<script type="text/javascript">
	jQuery(document).ready(function(){
		jQuery(".image_wrap").width(jQuery(".image_wrap img").width());
		jQuery(".price span").html(jQuery('.price_0').val());
		jQuery(".group2").colorbox({rel:'group2', transition:"fade"});
		jQuery(".fl").click(function(){
			if(jQuery('#overlay').css('display') == 'none'){
				jQuery('#overlay').fadeIn(100, function(){jQuery("#find-less").show();});
			}else{
				jQuery("#find-less").fadeOut(100, function(){jQuery("#overlay").hide();})
			}
		});
...
<?
/*if (!empty($arResult['ITEMS']) && count($arResult['SECTIONS']) == 0)
{
	$templateData = array(
