$(document).ready(function(){
	$('.depth-1.hasSublvl').hover(function(){
		$(this).addClass('ishover');
		var dkey = $(this).attr('data-key');
		setTimeout(function () {
			if($('.depth-1[data-key='+dkey+']').hasClass('ishover')){
				$('.depth-1[data-key='+dkey+']').addClass('hashover');
			}
		}, 300);
	},function(){
		$(this).removeClass('ishover').removeClass('hashover');
	});
});
...
function openMenu(id){
	...
	if(item.length > 0) {
		if(!item.hasClass('js-open')) {
			let items = item.siblings('.js-open');
			if(items.length > 0) {
				items.removeClass('js-open').removeClass('hashover').removeClass('ishover');
			}

			item.addClass('js-open').removeClass('hashover').removeClass('ishover');
		} else {
			item.removeClass('js-open').removeClass('hashover').removeClass('ishover');
		}
	}
}
...
function pndTopMenuSetHeight($iKey) {
	...
	if ($iKey != '' || $iKey != 'undefined') {
		$sel = '.is-head__catalog li[data-key="' + $iKey + '"]';
		if (jQuery($sel).children('.sublevels-wrapper').length > 0) {
			jQuery($sel).closest('.sublevels-wrapper').removeAttr('style');
			jQuery($sel).children('.sublevels-wrapper').css('height', '').removeClass('selected');
			...
			jQuery($sel).children('.sublevels-wrapper').css('height', '100%').addClass('selected');
		}
	}
}

function pndTopMenuResetHeight() {
	jQuery('.is-head__catalog ul.level-2 > li').each(function() {
		if (jQuery(this).children('.sublevels-wrapper').length > 0) {
			jQuery(this).closest('.sublevels-wrapper').removeAttr('style').removeClass('selected');
...
function pndTopMenuSetSelected() {
	jQuery.ajax({
		...
		jQuery('.is-head__catalog .sublevel a.item-selected').removeClass('item-selected');
		for($i = 0; $i < $res.length; $i++) {
			jQuery('.is-head__catalog .sublevel a[href="'+$res[$i]+'"]').addClass('item-selected');
		}
		if (jQuery('.is-head__catalog .sublevel a.item-selected').closest('li').hasClass('depth-3')) {
			jQuery('.is-head__catalog .sublevel a.item-selected').closest('li.depth-2').children('a:not(.item-selected)').addClass('item-selected');
		}
	});
}
/*
jQuery( document ).ready(function() {
	$hasL3 = false;
	jQuery('.is-head__catalog').on('mouseenter', 'li.parent', function() {
		...
		//	pndTopMenuSetSelected();
			...
			$lvl2MnW = 25;
			$zIndex = 1500;
			jQuery('.is-head__catalog .sublevel li').each(function() {
