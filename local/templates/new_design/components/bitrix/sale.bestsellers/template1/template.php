<?$arrFilter = Array("!PROPRTY_HITS" => false);?>
...
<div class="bx_item_list_bestsellers col<? echo $arParams['LINE_ELEMENT_COUNT']; ?> <? echo $templateData['TEMPLATE_CLASS']; ?>">
	<div class="cube-1">
		<img src="/bitrix/templates/eshop_adapt_blue/images/bg-top.png" alt="bg-top" />
		<div class="top_sale"><span class="block-title"><? echo GetMessage('SB_HREF_TITLE') ?>:</span></div>
		<div class="top_sale-link"><a href="/hits/">Все хиты</a></div>
	</div>
...
<? if (!empty($arResult['ITEMS'])): ?>
	...
	<a id="<? echo $arItemIDs['PICT']; ?>"
		...
		<? if ($arParams['SHOW_IMAGE'] == "Y")
		{
			...
			?>
			style="background-image: url(<? echo($arParams['SHOW_IMAGE'] == "Y" ? /*$arItem['PREVIEW_PICTURE']['SRC']*/ $img['src'] : ""); ?>)"
		<?
		} ?>
		title="<? echo $strTitle; ?>"><?
		...
		{
			?>
			<span class="bx_stick average left top"
				title="<? echo $arItem['LABEL_VALUE']; ?>"><? echo $arItem['LABEL_VALUE']; ?></span>
		<?
		}
		?>
	</a><?
