...
{	
	$rowData = array(
		...
		'REMOVE_NAME' => (isset($row['~NAME']) ? $row['~NAME'] : $row['NAME'])." ". $row['PROPERTY_SIZE_VALUE'].(!empty($row['PROPERTY_SIZE_VALUE']) ? ' размера' : ""),
	...
	$result['allSum_QUANTITY'] = (int)$result['allSum_QUANTITY']+(int)$row["QUANTITY"];
...
$totalData = array(
	...
	"TOTAL_QUANTITY" => $result['allSum_QUANTITY'],
	"TOTAL_QUANTITY_END" => theEnding($result['allSum_QUANTITY'])
);
