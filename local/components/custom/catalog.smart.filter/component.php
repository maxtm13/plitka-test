...
$is_ajax = false;
if((array_key_exists('GET_COUNT', $arParams) && $arParams['GET_COUNT'] === "Y") || (isset($_REQUEST["ajax"]) && $_REQUEST["ajax"] === "y")) {
    $is_ajax = true;
}
...
if ($this->StartResultCache(false, ['v10', $preFilter, ($arParams["CACHE_GROUPS"] ? $USER->GetGroups() : false)])) {
    ...
    if (!empty($arResult["ITEMS"])) {
        if ($this->facet->isValid()) {
            ...
            if ($this->arParams['SKIP_DATE_AND_PERMISSIONS_CHECK'] === 'Y') {
            ...
            if (!empty($this->arResult["PROPERTY_ID_LIST"])) {
                $rsElements = CIBlockElement::GetPropertyValues($this->IBLOCK_ID, $arElementFilter, false,
                    ['ID' => $this->arResult["PROPERTY_ID_LIST"]]);
                while ($arElement = $rsElements->Fetch()) {
                    $arElements[$arElement["IBLOCK_ELEMENT_ID"]] = $arElement;
                }
            } else {
                $rsElements = CIBlockElement::GetList(['ID' => 'ASC'], $arElementFilter, false, false,
                    ['ID', 'IBLOCK_ID']);
                while ($arElement = $rsElements->Fetch()) {
            ...
            if (!empty($arElements) && $this->SKU_IBLOCK_ID && $arResult["SKU_PROPERTY_COUNT"] > 0) {
                ...
                $rsElements = CIBlockElement::GetPropertyValues($this->SKU_IBLOCK_ID, $arSkuFilter, false,
                    ['ID' => $this->arResult["SKU_PROPERTY_ID_LIST"]]);
        ...
        foreach ($arResult["ITEMS"] as $PID => $arItem) {
            uasort($arResult["ITEMS"][$PID]["VALUES"], [$this, "_sort"]);
        }
    }
    ...
    if ($arParams["XML_EXPORT"] === "Y") {
        ...
        if ($this->SECTION_ID > 0) {
            $arSelect = ["ID", "IBLOCK_ID", "LEFT_MARGIN", "RIGHT_MARGIN"];
            ...
            if ($arResult["SECTION"]) {
                ...
                if ($arParams["SECTION_DESCRIPTION"] !== "") {
                    ...
                    $arResult["SECTION_DESCRIPTION"] = $obParser->html_cut($arResult["SECTION"][$arParams["SECTION_DESCRIPTION"]],
                        200);
                }
            }
        }
    ...
    foreach ($arItem["VALUES"] as $key => $ar) {
        ...
        if (
            ...
            if ($arItem["PROPERTY_TYPE"] == "N") {
                ...
                if ($arResult["FACET_FILTER"] && strlen($_CHECK[$ar["CONTROL_NAME"]]) > 0) {
                    if ($key == "MIN") {
                        $this->facet->addPriceFilter($arResult["PRICES"][$PID]["ID"], ">=",
                            $_CHECK[$ar["CONTROL_NAME"]]);
                    } elseif ($key == "MAX") {
                        $this->facet->addPriceFilter($arResult["PRICES"][$PID]["ID"], "<=",
                            $_CHECK[$ar["CONTROL_NAME"]]);
                    }
                ...
                if ($arResult["FACET_FILTER"] && strlen($_CHECK[$ar["CONTROL_NAME"]]) > 0) {
                    if ($key == "MIN") {
                        $this->facet->addDatetimePropertyFilter($PID, ">=",
                            MakeTimeStamp($_CHECK[$ar["CONTROL_NAME"]], FORMAT_DATE));
                    } elseif ($key == "MAX") {
                        $this->facet->addDatetimePropertyFilter($PID, "<=",
                            MakeTimeStamp($_CHECK[$ar["CONTROL_NAME"]], FORMAT_DATE) + 23 * 3600 + 59 * 60 + 59);
                    }
                }
...
foreach ($arResult["ITEMS"] as $PID => $arItem) {
    if (isset($arItem["PRICE"])) {
        if (strlen($arItem["VALUES"]["MIN"]["HTML_VALUE"]) && strlen($arItem["VALUES"]["MAX"]["HTML_VALUE"])) {
            ${$FILTER_NAME}["><CATALOG_PRICE_" . $arItem["ID"]] = [
        ...
        if ($existMinValue || $existMaxValue) {
            ...
            if ($existMinValue && $existMaxValue) {
                ...
                $filterValue = [$arItem["VALUES"]["MIN"]["HTML_VALUE"], $arItem["VALUES"]["MAX"]["HTML_VALUE"]];
        ...
        if ($existMinValue || $existMaxValue) {
            ...
            if ($existMinValue && $existMaxValue) {
                ...
                if ($timestamp1 && $timestamp2) {
                    $filterValue = [
                        FormatDate("Y-m-d H:i:s", $timestamp1),
                        FormatDate("Y-m-d H:i:s", $timestamp2 + 23 * 3600 + 59 * 60 + 59)
                    ];
                }
            } elseif ($existMinValue) {
                $filterKey = ">=PROPERTY_" . $PID;
        ...
        foreach ($arItem["VALUES"] as $key => $ar) {
            if ($ar["CHECKED"]) {
                ...
                $datetimeFilters[] = [$filterKey => $filterValue];
            }
        }
            ...
            if ($ar["CHECKED"]) {
                ...
                if ($arItem["IBLOCK_ID"] == $this->SKU_IBLOCK_ID) {
                    ...
                    if (!isset(${$FILTER_NAME}["OFFERS"][$filterKey])) {
                        ...
                        ${$FILTER_NAME}["OFFERS"][$filterKey] = [$filter[$filterKey], $backKey];
                    ...
                    if (!isset(${$FILTER_NAME}[$filterKey])) {
                        ...
                        ${$FILTER_NAME}[$filterKey] = [$filter[$filterKey], $backKey];
...
if ($arResult["FACET_FILTER"] && $this->arResult["CURRENCIES"]) {
    ...
    ${$FILTER_NAME}["FACET_OPTIONS"]["CURRENCY_CONVERSION"] = [
...
if ($arParams["SEF_MODE"] == "Y") {
    ...
    if ($this->SECTION_ID > 0) {
        $sectionList = CIBlockSection::GetList([], [
            ...
            "IBLOCK_ID" => $this->IBLOCK_ID,
            "ACTIVE" => "Y",
        ], false, ["ID", "IBLOCK_ID", "SECTION_PAGE_URL"]);
    ...
    if ($section) {
        ...
        $url = CIBlock::ReplaceSectionUrl($arParams["SEF_RULE"], []);
    }
...
$clearURL = CHTTP::urlDeleteParams($pageURL, $paramsToDelete, ["delete_system_params" => true]);

if ($arResult["JS_FILTER_PARAMS"]["SEF_SET_FILTER_URL"]) {
    $arResult["FILTER_URL"] = $arResult["JS_FILTER_PARAMS"]["SEF_SET_FILTER_URL"];
    $arResult["FILTER_AJAX_URL"] = htmlspecialcharsbx(CHTTP::urlAddParams($arResult["FILTER_URL"], [
    ...
    $arResult["FILTER_URL"] = htmlspecialcharsbx(CHTTP::urlAddParams($clearURL, $paramsToAdd, [
        "skip_empty" => true,
        "encode" => true,
    ]));

    $arResult["FILTER_AJAX_URL"] = htmlspecialcharsbx(CHTTP::urlAddParams($clearURL, $paramsToAdd + [
            "bxajaxid" => $_GET["bxajaxid"],
        ], [
        "skip_empty" => true,
        "encode" => true,
...
if ($is_ajax) {
    ...
    if(array_key_exists('GET_COUNT', $arParams) && $arParams['GET_COUNT'] === "Y") {

        $res_el = CIBlockElement::GetList([], $arFilter, false, false);

        $arResult_sections = [];
        $res_element = [];
        while($arfields = $res_el->GetNext()) {
            if(!in_array($arfields["IBLOCK_SECTION_ID"], $arResult_sections)) {
                $arResult_sections[] = $arfields["IBLOCK_SECTION_ID"];
            }
            $res_element[] = $arfields["ID"];
        }

        $arResult["SECTION_COUNT"] = count($arResult_sections);
        $arResult["ELEMENT_COUNT"] = count($res_element);
    } else {
        $arResult["ELEMENT_COUNT"] = CIBlockElement::GetList([], $arFilter, [], false);
    }
...
if (
    ...
    $exportUrl = CHTTP::urlAddParams($clearURL, ["mode" => "xml"]);
...
if(array_key_exists('GET_COUNT', $arParams) && $arParams['GET_COUNT'] === "Y") {
    ...
    return ["EL"=>$arResult["ELEMENT_COUNT"], "SEC"=>$arResult["SECTION_COUNT"]];
    ...
    //составим массив активных параметров
    global $arActiveFilterParams;
    $arActiveFilterParams = array();
    $key = 0;
    foreach ($arResult['ITEMS'] as $fParam) {
        if ((!empty($fParam['PRICE']) || $fParam['PROPERTY_TYPE'] == 'N') && (!empty($fParam['VALUES']['MIN']['HTML_VALUE']) || !empty($fParam['VALUES']['MAX']['HTML_VALUE']))) { //цена или числа
            $arActiveFilterParams[$key]['NAME'] = $fParam['NAME'];
            if (!empty($fParam['PRICE'])) {
                $arActiveFilterParams[$key]['PRICE'] = 1;
            }
            if (!empty($fParam['VALUES']['MIN']['HTML_VALUE'])) {
                $arActiveFilterParams[$key]['MIN']['VALUE'] = $fParam['VALUES']['MIN']['HTML_VALUE'];
                $arActiveFilterParams[$key]['MIN']['CONTROL_ID'] = $fParam['VALUES']['MIN']['CONTROL_ID'];
            }
            if (!empty($fParam['VALUES']['MAX']['HTML_VALUE'])) {
                $arActiveFilterParams[$key]['MAX']['VALUE'] = $fParam['VALUES']['MAX']['HTML_VALUE'];
                $arActiveFilterParams[$key]['MAX']['CONTROL_ID'] = $fParam['VALUES']['MAX']['CONTROL_ID'];
            }
            $key++;
        } else { //другие пар-ры, как правило, список
            foreach ($fParam['VALUES'] as $val) {
                if ($val['CHECKED']) {
                    $arActiveFilterParams[$key]['NAME'] = $val['VALUE'];
                    $arActiveFilterParams[$key]['CONTROL_ID'] = $val['CONTROL_ID'];
                    $key++;
                }
            }
        }
