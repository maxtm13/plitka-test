...
JCSmartFilter.prototype.click = function(checkbox)
{
	...
	if (jCheckBox){
		jCheckBox.parents('form').find('#set_filter').click();
	}
	if(this.timer)
		clearTimeout(this.timer);
	this.timer = setTimeout(BX.delegate(function(){
	...
	if(!!result && !!result.ITEMS)
	{
			...
			else if(arItem.VALUES)
			{
				for(var i in arItem.VALUES)
				{
					...
					if(control)
					{
						...
						let jControl = $(control);
						let jLabel = $('label[for='+ $(control).prop('id') +']');
						if (ar.DISABLED){
							jControl.prop('disabled',true);
							jLabel.addClass('disabled');
							jControl.parent().addClass('disabled');

						} else {
							jControl.prop('disabled',false);
							jLabel.removeClass('disabled');
							jControl.parent().removeClass('disabled');
						}
					}
				}
			}
		...
		if(modef && modef_num)
		{
			...
			else
			{
				...
				sel_param_pos = jQuery(window.curFilterinput).offset().top;
