...
class CBitrixCatalogSmartFilter extends CBitrixComponent
{
            ...
            if (self::$catalogIncluded) {
                ...
                while ($arPrice = $rsPrice->Fetch()) {
                    if ($arPrice["CAN_ACCESS"] == "Y" || $arPrice["CAN_BUY"] == "Y") {
                        ...
                        $error = "";
                        $utf_id = \Bitrix\Main\Text\Encoding::convertEncoding(toLower($arPrice["NAME"]), LANG_CHARSET,
                            "utf-8", $error);
                        $items[$arPrice["NAME"]] = [
                            "ID" => $arPrice["ID"],
                            "CODE" => $arPrice["NAME"],
    ...
    public function predictIBSectionFetch($id = [])
    {
        ...
        $link = CIBlockSection::GetList([], $arLinkFilter, false,
