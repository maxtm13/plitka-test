/*if ((navigator.userAgent.indexOf('Trident') != -1 || navigator.userAgent.indexOf('MSIE') != -1) && (window.location.search == '' || window.location.search == 'undefined')) {
	//ie и нет пар-ра 'v'
	window.location.replace(window.location.pathname+'?v='+Math.random());
}*/
...
function updateBasketTable(basketItemId, res)
{
		...
		for (id in res.BASKET_DATA.GRID.ROWS)
		{
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
				...
				if (BX('old_price_' + id)) {
					...
					if (item.FULL_PRICE_FORMATED != item.PRICE_FORMATED) { //скидка битрикса
						BX('old_price_' + id).innerHTML = item.FULL_PRICE_FORMATED;
					} else if (parseFloat(item.PROP_DISCOUNT_PERCENT) > 0) { //наша скидка из св-ва
						BX('old_price_' + id).innerHTML = item.OLD_PRICE_FORMATED;
					} else {
						BX('old_price_' + id).innerHTML = '';
					}
				}
				/*---end 2019-08-09---*/
	...
	if (!!res.BASKET_DATA)
	{
		...
		// console.log('he///', res['BASKET_DATA'].allSum);
		
		if (BX(document.querySelector('.rodobwna'))) {
			if (res['BASKET_DATA'].allSum >= 10000) {
				BX(document.querySelector('.rodobwna')).style.display = 'none'
				BX(document.querySelector('.bx_ordercart_order_pay_center')).style.display = 'block'
			} else {
				BX(document.querySelector('.rodobwna')).style.display = 'block'
				BX(document.querySelector('.bx_ordercart_order_pay_center')).style.display = 'none'
			}
...
function skuPropClickHandler(e)
{
	...
	if (!!target && target.hasAttribute('data-value-id'))
	{
		...
		BX.ajax({
			url: BX('tmpl_path').value+'/ajax.php', //'/bitrix/components/bitrix/sale.basket.basket/ajax.php', /*---bgn 2019-08-09---*/
			method: 'POST',
			data: postData,
			dataType: 'json',
...
function updateQuantity(controlId, basketId, ratio, bUseFloatQuantity)
{
	...
	else
	{
		...
		{
			BX(controlId).value = Math.round(oldVal);
		}
	}
	var getPrice = $("#current_price_" + basketId).attr("data-price");
	var setPrice = getPrice*newVal;
	setPrice = setPrice.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ');
	
	$('#item_sum_'+basketId).html(setPrice+" руб.");
}
