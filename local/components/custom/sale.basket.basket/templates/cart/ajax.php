...
/*---bgn 2019-08-09---*/
foreach ($arRes['BASKET_DATA']["GRID"]["ROWS"] as &$arItem) {
	...
	if (!empty($arE['PROPERTY_DISCOUNT_PERCENT_VALUE'])) { //есть скидка
		...
		if ($percent > 0) {
			...
			$arItem['PROP_DISCOUNT_PERCENT'] = $percent;
			...
			if (floatval($arRes['BASKET_DATA']["DISCOUNT_PRICE_ALL"]) > 0) {
				$arRes['BASKET_DATA']["DISCOUNT_PRICE_ALL"] += $economy * $arItem['QUANTITY'];
			} else {
				$arRes['BASKET_DATA']["DISCOUNT_PRICE_ALL"] = $economy * $arItem['QUANTITY'];
			}
			$arRes['BASKET_DATA']["DISCOUNT_PRICE_ALL_FORMATED"] = CurrencyFormat($arRes['BASKET_DATA']["DISCOUNT_PRICE_ALL"], 'RUB');
			$withoutDiscount = $arRes['BASKET_DATA']['allSum'] + $arRes['BASKET_DATA']["DISCOUNT_PRICE_ALL"];
			$arRes['BASKET_DATA']['PRICE_WITHOUT_DISCOUNT'] = CurrencyFormat($withoutDiscount, 'RUB');
		}		
	}
}
unset($arItem);
