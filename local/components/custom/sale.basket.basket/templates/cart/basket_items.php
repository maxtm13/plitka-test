				...
				?>
					<tr id="<?=$arItem["ID"]?>">
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								<td class="custom rbs-mobile-cell rbs-cell-qty">
							...
							elseif ($arHeader["id"] == "PRICE"):
echo "<!--<pre>";
print_r($arItem["PRICE"]);
echo "</pre>-->";

							?>
								<td class="price rbs-mobile-cell">
										...
										<div class="old_price" id="old_price_<?=$arItem["ID"]?>">
											...
											<?/*---bgn 2019-08-09---*/
											} else if (!empty($arItem['OLD_PRICE_FORMATED'])){
												echo $arItem['OLD_PRICE_FORMATED'];
											/*---bgn 2019-08-09---*/
											}?>
										</div>
										<?/*---bgn 2019-08-09---*/
										//нужно для рассчёта экономии в скрипте
										/*if (!empty($arItem['DISCOUNT_PERCENT'])){?>
											<span id="discount_percent_<?=$arItem["ID"]?>" style="display: none;"><?=$arItem['DISCOUNT_PERCENT']?></span>
										<?}*/
										/*---end 2019-08-09---*/?>
							...
							?>
								<td class="custom">
									<? if($arHeader["name"]):?><span><?=$arHeader["name"]; ?>:</span><? endif; ?>
							...
							?>
								<td class="custom rbs-mobile-cell rbs-cell-sum" id="item_sum_<?=$arItem["ID"]?>">
									<? if($arHeader["name"]):?><span><?=$arHeader["name"]; ?>:</span><? endif; ?>
									<?
									if ($arHeader["id"] == "SUM"):
									?>
						...
						?>
							<td class="control">
								...
								if ($bDelayColumn):
								?>
									<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?><?php /*if (substr_count($_SERVER['HTTP_USER_AGENT'], 'Trident') > 0 || substr_count($_SERVER['HTTP_USER_AGENT'], 'MSIE') > 0) { echo '&'.uniqid('v='); }*/ ?>"><?=GetMessage("SALE_DELAY")?></a>
								<?
								endif;
								?>
							</td>
	...
	<input type="hidden" id="tmpl_path" value="<?=$templateFolder?>" />
