		...
		<tbody>
			...
			?>
				<tr id="<?=$arItem["ID"]?>">
					...
					?>
						<td class="control">
							<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["add"])?><?php /*if (substr_count($_SERVER['HTTP_USER_AGENT'], 'Trident') > 0 || substr_count($_SERVER['HTTP_USER_AGENT'], 'MSIE') > 0) { echo '&'.uniqid('v='); }*/ ?>"><?=GetMessage("SALE_ADD_TO_BASKET")?></a><br />
							<?
							if ($bDeleteColumn):
							?>
